function [out] = bootstrap_All(report, SJ2data, SJ3data, TOJdata, ...
                               SampleSize, ConfCoef, FixedSeed)
%
% Obtain bootstrap confidence intervals (CIs) and p-values for model fitted jointly
% to data from the three tasks (SJ2, SJ3, and TOJ) through fit_All
%
% Usage: o = bootstrap_All(report, SJ2data, SJ3data, TOJdata, ...
%                          SampleSize, ConfCoef, FixedSeed)
% Arguments:
%
% - report:     Output structure returned by fit_All
%
% - SJ2data,
%   SJ3data,
%   TOJdata:    Data matrices given to fit_All, which produced the output structure
...
elseif ~isstruct(report)
  error('First argument must be an output structure from fit_All')
elseif ~strncmpi(report.Problem,'Joint fit of SJ2, SJ3, and TOJ',30)
  error('Input structure does not seem to come from fit_All')
...
LamR_Start = pars(2);
Tau_Start = pars(3);
Delta_Start = mean([pars(4) pars(8) pars(18)]);
Err_Start = 0.05;
Bias_Start = 0.5;
...
SJ2dat(1,:) = LevSJ2; SJ3dat(1,:) = LevSJ3; TOJdat(1,:) = LevTOJ;
epskap = [pars(5:7) pars(9:17) pars(19) pars(21)];
...
for i=1:SampleSize
  ...
  % simulate TOJ
  p1 = psi_TOJ_TF(LevTOJ,pars(1),pars(2),pars(3),pars(18),pars(20),epskap(13:14));
  sim = binornd(Trials_TOJ,p1);
  TOJdat(2,:) = sim;
  TOJdat(3,:) = Trials_TOJ-sim;
  t = fit_All(SJ2dat, SJ3dat, TOJdat, LamBounds, TauBounds, DeltaBounds, ...
...
% create report
out = struct('Problem', 'Bootstrap results for fit_All',...
             'Model', Model, ...
             ...
             'PE_CI_TOJ_Delta', CI(18,:), ...
             'PE_CI_TOJ_epsilonTF', CI(19,:), ...
             'PE_CI_TOJ_Xi', CI(20,:), ...
             'PE_CI_TOJ_epsilonRF', CI(21,:), ...
