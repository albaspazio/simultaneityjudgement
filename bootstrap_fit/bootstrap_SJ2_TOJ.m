function [out] = bootstrap_SJ2_TOJ(report, SJ2data, TOJdata, ...
                                   SampleSize, ConfCoef, FixedSeed)
%
% Obtain bootstrap confidence intervals (CIs) and p-values for model fitted jointly
% to data from SJ2 and TOJ tasks through fit_SJ2_TOJ
%
% Usage: o = bootstrap_SJ2_TOJ(report, SJ2data, TOJdata, ...
%                              SampleSize, ConfCoef, FixedSeed)
% Arguments:
%
% - report:     Output structure returned by fit_SJ2_TOJ
%
% - SJ2data,
%   TOJdata:    Data matrices given to fit_SJ2_TOJ, which produced the output structure
...
elseif ~isstruct(report)
  error('First argument must be an output structure from fit_SJ2_TOJ')
elseif ~strncmpi(report.Problem,'Joint fit of SJ2 and TOJ',24)
  error('Input structure does not seem to come from fit_SJ2_TOJ')
...
SJ2dat(1,:) = LevSJ2; TOJdat(1,:) = LevTOJ;
epskap = [pars(5:7) pars(9) pars(11)];
...
for i=1:SampleSize
  ...
  % simulate TOJ
  p1 = psi_TOJ_TF(LevTOJ,pars(1),pars(2),pars(3),pars(8),pars(10),epskap(4:5));
  sim = binornd(Trials_TOJ,p1);
  TOJdat(2,:) = sim;
  TOJdat(3,:) = Trials_TOJ-sim;
  t = fit_SJ2_TOJ(SJ2dat, TOJdat, LamBounds, TauBounds, DeltaBounds, ...
...
% create report
out = struct('Problem', 'Bootstrap results for fit_SJ2_TOJ',...
             'Model', Model, ...
             ...
             'PE_CI_TOJ_Delta', CI(8,:), ...
             'PE_CI_TOJ_epsilonTF', CI(9,:), ...
             'PE_CI_TOJ_Xi', CI(10,:), ...
             'PE_CI_TOJ_epsilonRF', CI(11,:), ...
